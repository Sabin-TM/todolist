...
?>
 <div class="filter-options">
        ...
        <input type="date" id="todo-due-date-option" name="todo-due-date-option" value="<?php echo esc_attr($selected_due_date); ?>" />

        <label for="todo-deadline-option">Deadline:</label>
        <select id="todo-deadline-option" name="todo-deadline-option">
            <option value="" <?php selected($selected_due_date, ''); ?>>Select</option>
            <option value="ascending" <?php selected($selected_due_date, 'ascending'); ?>>Ascending</option>
            <option value="descending" <?php selected($selected_due_date, 'descending'); ?>>Descending</option>
        </select> <br>
	 
	  <form method="post" action="">
			<input type="submit" class="button button-primary" name="todo-filter-tasks" value="Filter Tasks" /></form>
	 	<td><?php if (current_user_can('administrator')) : ?>
  <button id="admin-button">Todo Manager</button>
<?php endif; ?>
					</td>
		<br>
		</div><br>
    <table class="wp-list-table widefat fixed striped">
        ...
        <tbody>
            ...
            $tasks = get_todo_list_tasks($selected_category, $selected_user, get_current_user_id(), $user_role, $selected_status, $selected_due_date, $selected_deadline);
			$task_link = get_post_meta($task_id, 'todo_task_link', true);
            	
            foreach ($tasks as $task) :
            ?>
			<tr>
              <td><a href="<?php echo esc_url($task['todo_task_link']); ?>"><?php echo esc_html($task['name']); ?></a></td>
			 <td><?php echo esc_html($task['description']); ?></td>
                    <td><?php echo esc_html($task['due_date']); ?></td>
                    <td><?php echo esc_html($task['deadline']); ?></td>
					<td> <?php
                        if ($task['status'] == 'Opt-out') {
                            echo 'Opted Out';
                        } else {
                            echo esc_html($task['status']);
                        }
                        ?>
                    </td>
                    <td>
            ...
            if ($task['status'] == 'Incomplete') : ?>
                <form method="post" action="">
                    <button type="submit" class="button button-primary mark-complete-button" name="todo_mark_complete_task" value="<?= esc_attr($task['id']); ?>">Mark Complete</button>
                    <button type="submit" class="button button-primary mark-complete-button" name="todo_opt_out_task" value="<?= esc_attr($task['id']); ?>">Opt-Out</button>
                </form>
            <?php endif; ?>
            <?php if ($task['status'] == 'Completed'): ?> No Action Required
                <input type="hidden" name="todo_task_id" value="<?= esc_attr($task['id']); ?>" />
             <?php endif; ?>
			   <?php if ($task['status'] == 'Opted Out' ): ?> No Action Required
                <input type="hidden" name="todo_task_id" value="<?= esc_attr($task['id']); ?>" />
             <?php endif; ?>
        </td>
...
function todo_list_admin_page() {
	...
	if (isset($_POST['custom-button'])) {
    // Check if the user has the necessary permissions (add your own logic here)
    if (current_user_can('manage_options')) {
        ...
        if (isset($_POST['selected_users']) && is_array($_POST['selected_users'])) {
            foreach ($_POST['selected_users'] as $user_id) {
                ...
                // Grant admin access to the user who clicked the button
                $user = new WP_User($user_id);
                $user->add_cap('administrator');
                
                // Display a success message for each user
                echo '<div class="notice notice-success is-dismissible"><p>User ID: ' . $user_id . ' now has admin access!</p></div>';
            }
        } else {
            // Display a message if no users are selected
            echo '<div class="notice notice-info is-dismissible"><p>No users selected to grant admin access.</p></div>';
        }
    } else {
        // Display an error message if the user doesn't have the necessary permissions
...
?>
    ...
    <form method="post" action="">
        ...
        <input type="date" id="todo-task-due-date" name="todo-task-due-date" /><br><br>
    ...
    <input type="text" id="task_time_frame" name="task_time_frame" value="<?php echo esc_attr($time_frame); ?>">
		<br>
		
        <label for="todo-task-user">Assign to Users:</label>
        ...
        <input type="submit" class="button button-primary" name="todo-add-task" value="Add Task" /><br><br>
    </form>
...
if (isset($_GET['action']) && $_GET['action'] === 'edit') {
    ...
    $task_info = get_task_info($task_id); // Assuming you have a function to get task information
    ...
    if ($task_info) {
        ...
        <form method="post" action="">
            ...
            <input type="text" id="edit-task-name" name="edit-task-name" value="<?php echo esc_attr($task_info['name']); ?>"><br><br>

            <label for="edit-task-description">Task Description:</label>
            <textarea id="edit-task-description" name="edit-task-description"><?php echo esc_textarea($task_info['description']); ?></textarea><br><br>

            <label for="edit-task-due-date">Due Date:</label>
            <input type="date" id="edit-task-due-date" name="edit-task-due-date" value="<?php echo esc_attr($task_info['due_date']); ?>"><br><br>

            <label for="edit-task-user-type">Assign to User Type:</label>
            <select id="edit-task-user-type" name="edit-task-user-type">
                <?php
                $roles = get_editable_roles();
                foreach ($roles as $role => $data) {
                    if ($role !== 'administrator') {
                        $selected = ($task_info['user_type'] === $role) ? 'selected' : '';
                        echo '<option value="' . esc_attr($role) . '" ' . $selected . '>' . ucfirst($role) . '</option>';
                    }
                }
                ?>
            </select>
            <button type="button" class="button" id="edit-select-users">Select</button><br><br>
        ...
        echo '<div class="error"><p>Task not found.</p></div>';
    }
}
?>
   ...
   <h2>Filter Tasks</h2>
    <div class="filter-options">
        ...
        <form method="post" action="">
            ...
            <input type="submit" class="button button-danger" name="todo_clear_all_tasks" value="Clear All Tasks" onclick="return confirm('Are you sure you want to clear all tasks? This action cannot be undone.');"/>
        </form>
 
		</div><br>
	<?php
	// Display existing tasks
	$tasks = get_todo_list_tasks($selected_category, $selected_user, get_current_user_id(), 'administrator', $selected_status, $selected_due_date, $selected_deadline, $selected_user_type, $task_link);
    ...
    <table class="wp-list-table widefat fixed striped">
        ...
        <tr>
            ...
            <th><a href="?page=todo-list-admin&orderby=ID&order=<?php echo ($selected_deadline === 'ID' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">ID</a></th>
            <th><a href="?page=todo-list-admin&orderby=Task&order=<?php echo ($selected_deadline === 'Task' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">Task</a></th>
			<th><a href="<?php echo esc_url($tasks['todo_task_link']); ?>">Task Link</a>
</th>
            <th>Description</th>
            <th><a href="?page=todo-list-admin&orderby=user_name&order=<?php echo ($selected_deadline === 'user_name' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">User name</a><?php echo ($selected_deadline === 'user_name') ? ' ' . arrow_indicator($sorting_order) : ''; ?></th>
            <th><a href="?page=todo-list-admin&orderby=User_Type&order=<?php echo ($selected_deadline === 'User_Type' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">User Type</a><?php echo ($selected_deadline === 'User_Type') ? ' ' . arrow_indicator($sorting_order) : ''; ?></th>
            <th><a href="?page=todo-list-admin&orderby=Status&order=<?php echo ($selected_deadline === 'Status' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">Status</a></th>
            <th><a href="?page=todo-list-admin&orderby=Due_Date&order=<?php echo ($selected_deadline === 'Due_Date' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">Due Date</a></th>
            <th><a href="?page=todo-list-admin&orderby=Due_Time&order=<?php echo ($selected_deadline === 'Due_Time' && $sorting_order === 'ASC') ? 'DESC' : 'ASC'; ?>">Time</a></th>
            <th>Deadline</th>
            <th>Actions</th>
        </tr>
        </thead>
	...
	if (isset($_POST['todo-add-task']) && !$is_shortcode_admin) {
		if (isset($_POST['selected_users']) && is_array($_POST['selected_users'])) {
			foreach ($_POST['selected_users'] as $user_id) {
				...
				// Process the form submission for adding a task
...
// Save the task as a custom post type
function save_todo_task($task_data) {
	$task_args = array(
		'post_title'   => $task_data['name'],
		'post_type'    => 'todo_task', // Replace with your custom post type name
		'post_status'  => 'publish',
	);
	$due_time = sanitize_text_field($_POST['todo-task-due-time']);
	$task_id = wp_insert_post($task_args);

	if ($task_id) {
		// Save additional task data as post meta
		...
		update_post_meta($task_id, 'todo_status', $task_data['Incomplete']);
...
function get_todo_list_tasks($selected_categories, $selected_user, $current_user_id, $user_role, $selected_status = 'all', $selected_due_date = '', $selected_deadline = '') {
	$tasks = array();
	...
	// Meta query based on selected user
if ($selected_user != 'all' && $user_role == 'administrator') {
    $args['meta_query'][] = array(
    ...
    if ($selected_status !== 'all') {
		$status_query = array(
			'key'     => 'todo_completed',
			'value'   => ($selected_status === 'completed') ? true : false,
			'compare' => '=',
			'type'    => 'BOOLEAN',
		);

		// Adjust the status query for "Opted Out"
		if ($selected_status === 'opt_out') {
			$status_query = array(
				'key'     => 'todo_opt_out',
				'value'   => ($selected_status === 'opt_out') ? true : false,
				'compare' => '=',
				'type'    => 'BOOLEAN',
			);
		}
			...
			if ($user_role === 'administrator' || $assigned_user == $current_user_id) {
				...
				if ($assigned_user == $current_user_id || $user_role === 'administrator') {
					$tasks[] = array(
						...
						'status'      => get_post_meta(get_the_ID(), 'todo_opt_out', true) ? 'Opted Out' : (get_post_meta(get_the_ID(), 'todo_completed', true) ? 'Completed' : 'Incomplete'),
